...
pub struct Function {
    ...
    pub fn call(&self, args: Vec<String>) -> Result<String, String> {
        ...
        if self.code.starts_with("##") {
            match self.code.get(2..).expect("What happened.") {
                "sin" => {
                    match args.pop() {
                        Some(arg) => {
                            match arg.parse::<f64>() {
...
fn test_function_new() {
    ...
    eprintln!("{:?}", sin.call(vec![format!("{}", (90.0f64).to_radians())]));
    let foo = Function::new("function foo a b = 2 * a + b".to_owned()).unwrap();
    eprintln!("{:?}", foo.call(vec!["10".to_owned(), "11".to_owned()]));
    eprintln!("{:?}", foo.call(vec!["a".to_owned(), "b".to_owned()]));
}