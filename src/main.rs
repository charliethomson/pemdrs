...
enum Operator {
    ...
    fn eval(&self, lhs: usize, rhs: usize) -> usize {
        match self {
            Operator::Pow => { lhs.pow(rhs as u32) },
...
fn main() -> Result<(), Error> {
    ...
    eprintln!("Original: {} \nTokenStream: {}", &s, TokenStream::from_string(&mut s.clone())?); 

    eprintln!("Original: {} \nShunting yard'd: {}", &s, shunting_yard(&TokenStream::from_string(&mut s.clone())?)?);
    Ok(())
}