...
struct Token {
    ...
    fn is_side_paren(&self, side: Paren) -> bool {
        match self.is_paren() {
            Some(n) => { return n == side; },
            None => { return false; }
        }
    }
...
fn shunting_yard(expr: &mut Expression) -> Result<Expression, String> {
    ...
    'main: for token in expr.tokens() {
        eprintln!("Main loop: Output: {}", output.to_string());
        if token.is_numeric() {
            output.add_token(token.clone());
        } else if token.is_operator() {
            // opstack.push_back(token.clone());
            ...
            /* if it's an operator but not a paren */
                ...
                'check: while let Some(stacktop) = top(&mut opstack) {
                    /* if the item on top is an opening paren */
                    if stacktop.is_side_paren(Paren::Open) {
                        /* break out of the loop */
                        check = true;
                    /* if the item on top has a greater precedence and is not an opening paren */
                    } else if stacktop.precedence() > token.precedence() {
                        /* break out of the loop */
                        check = true;
                    /*  if the item on top has equal precedence, is left associative and is not an opening paren */
